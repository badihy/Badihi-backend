...
export class PaginationProvider {
  async paginate<T>(
    ...
    const searchIn = Array.isArray(rawSearchIn)
      ? rawSearchIn
      : rawSearchIn
        ? [rawSearchIn]
        : [];
    ...
    if (sortBy) {
      const sortDirection: SortOrder =
        String(sortOrder).toLowerCase() === 'desc' || sortOrder === -1 ? -1 : 1;
  ...
  createOptionsFromQuery(query: any): PaginationOptions {
    ...
    if (query.searchIn) {
      options.searchIn = Array.isArray(query.searchIn)
        ? query.searchIn
        : [query.searchIn];
    }

    if (query.relations) {
      options.relations = Array.isArray(query.relations)
        ? query.relations
        : [query.relations];
    }
